...
- 本版本没有 Developer ID 签名，也未经过 Apple 公证；根据构建钥匙串可能是 ad-hoc 或本地开发自签名。使用“仍要打开”前请核对 SHA-256。
...
- `aacc-run` 可在 SIGINT/SIGTERM 后清理派生的进程树（进程组），但无法承诺 SIGKILL、断电或系统崩溃后的清理。
...
- OpenCode 任务状态依据本机部件快照推断（官方 idle/busy 为运行时事件不落库），采用 90 秒活动窗口；状态为近似。权限请求同样不落库，AACC 无法区分"等待批准的工具调用"和"仍在生成中的工具调用"，因此 OpenCode 卡片永不显示"等待同意"。Windows 优先发现 `%LOCALAPPDATA%\opencode\opencode.db`，并支持用户目录下的 XDG 回退位置；其他 OpenCode 发布通道数据库名只按固定允许列表搜索。
- AACC 隐藏 Qwen 刷新所使用的阿里云服务端会话生命周期很短（实测从数分钟到数小时不等，取决于阿里云风控）。AACC 会通过同步日常浏览器的百炼会话或提示重新登录来自愈，但无法延长服务端有效期，因此两次成功刷新之间，额度仍可能回落到「点击授权」。
