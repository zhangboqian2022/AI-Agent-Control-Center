...
Kimi shows `5H`, `WEEK`, and `MONTH`. The operating system's native per-application WebView store retains the first-party Kimi session. For native website-session reuse, AACC stores a protected reuse decision and does not copy cookies, passwords, a website bearer token, account names, or quota values into that gate. Kimi Code OAuth credentials are stored separately under AACC's existing credential protection. One coordinator starts the web source and Kimi Code fallback in the same five-minute cycle; Kimi Code can fill a fresh, temporarily missing `5H` or `WEEK`, but never invents `MONTH`. Quota lookups are metadata-only, send no prompt, and use no generation tokens. A known percentage without a trustworthy reset time remains visible while its reset displays `--`. Explicit logout synchronously disables reuse before attempting bounded native site-data cleanup. Native persistence and logout across restart remain subject to macOS and Windows manual sign-off.
