...
- **Sync-first login.** Clicking authorize no longer opens the visible Chrome
  login window immediately. AACC first silently syncs the daily Chrome Bailian
  session into the AACC-owned profile and fetches the quota hidden. The
  visible login window opens only when the silent sync fails to produce a live
  session, so a healthy daily session now turns the quota bar green without
  any window at all.
...
- **Robust login cancel.** The login watcher no longer treats cross-origin
  ...
  the 14-second auto-cancel fires only when every page target of the owned
  browser is gone, so completing a verification flow can no longer be
  misreported as a cancelled login.
...
- **Quota-bar status texts.** The quota bar shows bilingual status texts for
  session syncing and session-expired auto-recovery, so background recovery is
  visible instead of looking like a frozen reading.
...
- **同步优先登录。** 点击授权不再立即打开可见的 Chrome 登录窗。AACC 先静默
  把日常 Chrome 的百炼会话同步进 AACC 专属 profile 并隐藏拉取额度；只有静默
  同步没能拿到可用会话时才打开可见登录窗。日常会话健康时，额度栏直接变绿，
  全程无窗口。
...
- **自动居中登录弹窗。** 会话确认失效且所有自动恢复手段用尽时，AACC 会自动
  ...
  分钟最多一次；用户主动退出登录后绝不自动弹窗。
- **登录窗居中 + 提前注入反检测。** 可见登录窗现在居中打开，并在登录页加载前
  注入反检测（stealth）脚本。
- **稳健的登录取消判定。** 登录监视不再把跨域跳转（短信验证、风控验证）当作
  登录已关闭：14 秒自动取消只在托管浏览器的所有页面目标全部消失时触发，完成
  验证流程不会再被误报为取消登录。
- **零残留保证。** 每次操作（登录、隐藏刷新、重复制、登出、关闭）结束后，
  AACC 都会校验专属 profile 没有任何残留 Chrome 进程，并对幸存者强制终止，
  隐藏浏览器不会比创建它的操作活得更久。
- **额度栏状态文案。** 额度栏新增双语状态文案：会话同步中、会话已过期自动
  恢复中——后台恢复过程可见，不再像读数卡死。

## Verification / 验证记录

Local build and smoke verification results (bundle build, targeted Qwen tests,
full pytest suite, ruff/mypy, fresh launch of the replacement app) are pending
and will be recorded here after the release build completes.

本机构建与冒烟验证结果（应用构建、Qwen 针对性测试、全量 pytest、ruff/mypy、
替换后应用的新鲜启动）待发布构建完成后回填。
