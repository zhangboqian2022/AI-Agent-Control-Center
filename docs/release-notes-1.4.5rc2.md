...
- **Login via a real Chrome (CDP) on macOS.** Aliyun's sign-in (password /
  ...
  completes normally; the 5-minute quota refreshes then run headless.
  Cookies stay in `~/Library/Application Support/AACC/qwen-chrome-profile`;
...
- **Refreshes actually refresh.** OpenCode and Qwen quota refreshes now
  reload the workspace page on every 5-minute tick before extracting
  (previously the script re-read the stale DOM and values never changed);
  Kimi is unchanged (it fetches a live API).
...
- **macOS 登录改走真实 Chrome（CDP）。** 阿里云登录（密码 / 扫码 / RAM）
  ...
  登录——包括 RAM 登录——按正常浏览器流程完成；之后的每 5 分钟额度
  刷新走 headless。cookie 保存在
...
- **定期刷新真正生效。** OpenCode 与 Qwen 额度每 5 分钟轮询时先整页
  重载工作区页面再提取（此前只重读旧 DOM，数值永远不变）；Kimi 不变
  （其取数为实时 API）。
