...
AACC 默认每两秒读取本机 Codex 的任务索引。点击齿轮的“选择监控的 Codex 任务”，只有被勾选的任务才会显示、写入状态并检查活动；未勾选任务只保留在选择器列表中，不进行状态监控。它只读取任务 ID、标题、更新时间、会话文件修改时间和 PID，不读取对话内容、提示词、代码或命令。会话文件在 90 秒内有更新时显示“执行中”；PID 匹配时也显示“执行中”；其他已选任务会诚实显示“状态未知”。
