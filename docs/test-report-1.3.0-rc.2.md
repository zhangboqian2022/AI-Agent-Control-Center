...
- `uv run pytest -q`：连续两轮通过，每轮 196 项测试全部通过。
- 安装器在真实安装前再次运行全套测试：196 项全部通过。
- Codex 发现与服务定向测试：39 项通过，覆盖 5 秒默认间隔、安全短概括、真实审批事件、整数时间戳、长会话起点恢复、并发半行、截断重写、有界大行扫描、等待任务优先、完成事件与发现告警。
- 状态机与模型定向测试：16 项通过，覆盖短回合持续计时、等待状态、完成冻结，以及终态后直接进入运行或等待状态时重新归零。
- GUI 完整定向测试：26 项通过，覆盖横向卡片、长标题省略、任务选择、移除、自适应高度、80% 高度上限、内部滚动，以及关闭数据库时停止排队刷新。
- 隐私回归测试确认命令原始参数不会用于分类或进入 UI；只有显式安全类别和固定工具事件名可生成短状态概括。
...
- 安装后只运行一个 AACC 进程，稳定检查时 RSS 约 44 MB。
