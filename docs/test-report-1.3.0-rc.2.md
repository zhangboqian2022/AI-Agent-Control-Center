...
- `uv run pytest -q`：通过，182 项测试全部通过。
- 安装器在真实安装前再次运行全套测试：182 项全部通过。
- Codex 发现与服务定向测试：28 项通过，覆盖 5 秒默认间隔、安全短概括、格式异常、完整任务起始时间、等待确认、完成事件与发现告警。
- 状态机与模型定向测试：13 项通过，覆盖短回合持续计时、等待状态、完成冻结与终态后重新运行归零。
...
- 安装后只运行一个 AACC 进程，稳定检查时 RSS 约 51 MB。
