...
def test_dmg_build_targets_desktop_and_contains_app_bundle() -> None:
    ...
    assert "AACC-1.3.3-rc.1.dmg" in script
...
def test_release_version_is_consistent_across_project_and_build_scripts() -> None:
    assert __version__ == "1.3.3rc1"
    assert 'version = "1.3.3rc1"' in (ROOT / "pyproject.toml").read_text(encoding="utf-8")
    assert 'AACC_VERSION="${AACC_VERSION:-1.3.3-rc.1}"' in (
