...
The Codex quota strip is weekly-only. Its primary source starts the installed Codex `app-server` locally and calls only the read-only `account/rateLimits/read` method using the account already configured in Codex; it does not start a task, send a prompt, or initiate a login. AACC rediscovers that live source every 60 seconds, so ChatGPT/Codex opened or restarted after AACC can synchronize without restarting AACC; click the strip for an immediate refresh. AACC accepts only a future 10080-minute window. If that method or executable is unavailable, it falls back to bounded tails of recent local session files and ignores legacy shorter windows. No raw response, prompt, or task content is retained. Missing, expired, or changed metadata is shown as `--` rather than `0%`.
