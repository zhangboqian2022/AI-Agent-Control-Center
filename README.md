...
- **Cached Kimi membership quota.** The operating system's native per-application WebView store retains Kimi's first-party session. For native website-session reuse, AACC stores only a protected reuse decision and does not copy a cookie, password, website bearer token, account name, or quota value into that gate; Kimi Code OAuth credentials are stored separately and are not part of this web reuse gate. The web source and Kimi Code fallback start in the same five-minute cycle. These metadata-only lookups send no prompt and use no generation tokens. Explicit logout synchronously disables reuse, then attempts bounded native site-data cleanup.
...
The Codex quota strip is weekly-only. Its primary source starts the installed Codex `app-server` locally and calls only the read-only `account/rateLimits/read` method using the account already configured in Codex; it does not start a task, send a prompt, or initiate a login. AACC accepts only a future 10080-minute window. If that method or executable is unavailable, it falls back to bounded tails of recent local session files and ignores legacy shorter windows. No raw response, prompt, or task content is retained. Missing, expired, or changed metadata is shown as `--` rather than `0%`.

Kimi renders `5H`, `WEEK`, and `MONTH`. A known percentage with no trustworthy reset time keeps the percentage and shows `--` for its reset. Native-session persistence and logout across restart still require macOS and Windows manual sign-off before 1.4.2 can be released.
