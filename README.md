...
- **OpenCode Go-plan quota bar.** A self-owned macOS web view signs you into opencode.ai (GitHub/Google), extracts the rendered quota bars from the /go workspace page, and displays rolling/weekly/monthly quota (percentage + reset countdown) in a three-row strip. Cookie persists across AACC restarts; no prompt, reply, tool command, or reasoning content is ever read. Configure `opencode_workspace_url` in `config.yaml`.
