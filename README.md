...
[中文文档](README.zh-CN.md) · [Download AACC 1.4.6-rc.1](https://github.com/zhangboqian2022/AI-Agent-Control-Center/releases/tag/v1.4.6-rc.1) · [Release notes](docs/release-notes-1.4.6rc1.md) · [Product design](docs/product-design.md)
...
- **Quota resets at a glance.** Codex shows its 10080-minute `WEEK` window; Kimi shows `5H`, `WEEK`, and `MONTH`. OpenCode shows the Go-plan rolling/weekly/monthly quota. Qwen Code shows the Aliyun Bailian token-plan personal 5-hour/7-day windows and the team total. Every available reset is an absolute local date and time inside the row; a real `0%` stays `0%`, and only unknown data becomes `--`.
...
- **Qwen Code (Aliyun Bailian) quota bar.** The token-plan console's personal 5-hour / 7-day windows and team-plan total are rendered by an AACC-owned Google Chrome profile driven over CDP, refreshing every 15 minutes. Clicking authorize first reuses your daily Chrome's live Bailian login silently when `qwen_auto_session_recopy` is enabled and that browser is signed in to Bailian; only if that fails does a centered Chrome window open for the password/QR/SMS sign-in. Aliyun expires web sessions server-side, so AACC self-heals by re-syncing or reopening the login window at most once per 30 minutes — and closing a popped window stops automatic popups for the run. Only the rendered quota text is read; prompts, replies, and the password vault are never touched, and a session sync copies just the minimal Bailian session set into AACC's own protected profile directory. Configure `qwen_workspace_url` in `config.yaml`.
- **Qwen Code CLI task discovery.** Sessions are read from `~/.qwen/projects/<encoded path>/chats/*.jsonl`; the sibling `<uuid>.runtime.json` supplies pid and work directory, validated against the live process command line so a recycled pid can never fake “running”, and without a runtime file recent chat activity inside a 90-second window marks the session active. Token usage (input/output/cache) is aggregated from local session records.
...
Download [AACC-1.4.6-rc.1.dmg](https://github.com/zhangboqian2022/AI-Agent-Control-Center/releases/download/v1.4.6-rc.1/AACC-1.4.6-rc.1.dmg), open it, and drag `AACC.app` to Applications.
...
```bash
shasum -a 256 AACC-1.4.6-rc.1.dmg
```
...
The primary Windows installer is `AACC-1.4.5-Setup.exe` with its matching `AACC-1.4.5-Setup.exe.sha256`. No Windows build has been published for 1.4.6-rc.1 yet: Windows builds must be produced on a Windows machine and will be attached to the [v1.4.6-rc.1 release](https://github.com/zhangboqian2022/AI-Agent-Control-Center/releases/tag/v1.4.6-rc.1) once built. The 1.4.5 Setup is itself built on a Windows machine and attached to the v1.4.5 release after it; until any of those is available, use the Windows assets from the [v1.4.4-rc.1 release](https://github.com/zhangboqian2022/AI-Agent-Control-Center/releases/tag/v1.4.4-rc.1).
