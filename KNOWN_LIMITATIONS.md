...
- This release has no Developer ID signature and is not Apple-notarized. Depending on the build keychain it may be ad-hoc or local-development self-signed; verify its SHA-256 before using **Open Anyway**.
...
- `aacc-run` cleans up the spawned process tree (process group) after SIGINT/SIGTERM but cannot guarantee cleanup after SIGKILL, power loss, or an operating-system crash.
...
- OpenCode task status is inferred from local part snapshots (official idle/busy is a runtime event, not persisted) with a 90-second activity window; status is approximate. Permission requests are not persisted either, so AACC cannot distinguish a tool call waiting for approval from one still being generated, and OpenCode cards never report waiting-for-approval. Windows database discovery prefers `%LOCALAPPDATA%\opencode\opencode.db` and supports an XDG-compatible profile fallback; alternate OpenCode release-channel database names are searched in a fixed allowlist.
- The Aliyun server-side sessions used by AACC's hidden Qwen refresh are short-lived (observed anywhere from minutes to a few hours, depending on Aliyun risk control). AACC self-heals by syncing the daily browser's Bailian session or by asking to re-login, but it cannot extend the server-side lifetime, so a quota reading can still lapse into "click to authorize" between successful refreshes.
