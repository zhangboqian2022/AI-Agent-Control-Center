...
[中英文发布说明](docs/release-notes-1.4.6rc1.md)

- [新功能] Qwen 登录改为「同步优先」：点击授权后先用 AACC 专属 profile 里已有的会话隐藏拉取额度，只有确认该缓存失效时才把日常 Chrome 的百炼会话同步进去并重试；只有静默同步没能拿到可用会话时，才打开可见登录窗。同步优先与日常会话自愈都依赖 `qwen_auto_session_recopy`（默认关闭；因会把日常浏览器的百炼会话 Cookie 复制进 AACC 自有 profile，需用户主动开启）。
- [修复] 来源感知的受保护自愈：手动登录产生的会话在撞上登录横幅后先等 60 秒复核，确认过期才允许会话重复制覆盖。修复了一次横幅观测就摧毁刚登录 9 分钟的新会话的事故。
- [新功能] 会话确认失效且自动恢复手段用尽时，AACC 会自动弹出居中的 Qwen 登录窗，频率限制为每 30 分钟最多一次；用户主动退出登录后绝不自动弹窗。关闭 AACC 自动弹出的登录窗即视为拒绝：本次运行不再自动弹窗。
- [修复] Qwen 可见登录窗现在居中打开，并在登录页加载前注入反检测（stealth）脚本。
- [修复] 更稳健的登录取消判定：短信验证、风控验证等跨域跳转不再触发修复前约 14 秒就误报的自动取消；现在只有托管浏览器的所有页面目标连续 3 次轮询全部消失（并有 30 秒启动宽限）才判定登录被取消。
- [修复] 零残留保证：每次操作结束后校验 AACC 专属 profile 没有任何残留 Chrome 进程，发现幸存者立即强制终止。
- [新功能] 额度栏新增双语状态文案：会话同步中、同步失败即将打开登录窗、会话已过期自动恢复中。

## 1.4.5 — 2026-08-29

[中英文发布说明](docs/release-notes-1.4.5.md)

- [修复] Kimi 后台额度刷新不再因第一次 token 过期（401）而永久停摆：后台 401 现在会先走有界自恢复（重拉 → 整页重载 → 再拉）再判定登出。登录对话框保持已渲染页面稳定，改为轮询等待用户在页面中登录，不再反复重载。
- [修复] kimi.com 不再回报"页面加载完成"事件（一个长连接让每次加载都无法结束），导致整条 Kimi 管线停滞：现在导航开始 10 秒后即调度拉取脚本，页内脚本最多等待 25 秒让 SPA 引导写入 token。
- [修复] Kimi 登录对话框标记为置顶显示，菜单栏应用的窗口不会再被其他窗口遮挡；macOS 启动看门狗不再杀死一个加载完成事件永远不会到来的进行中刷新。
- [修复] OpenCode 额度提取恢复语言无关并支持一位小数百分比：2026-08-25 的站点改版破坏了旧的纯整数"NN%"匹配，而 zh-CN webview 渲染的是本地化标签。工作区被重定向到 auth.opencode.ai 时现在如实报告为未登录，不再空耗看门狗；DOM 超时会携带页面实际渲染文本写入日志。
- [修复] Qwen 可见登录会评估每一个百炼 CDP target 直到某个页面渲染出额度，登录重定向遗留的陈旧未登录标签页不会再把流程饿死到超时。
