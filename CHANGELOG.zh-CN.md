...
- [新功能] Qwen 登录改为「同步优先」：点击授权后先静默把日常 Chrome 的百炼会话同步进 AACC 专属 profile 并隐藏拉取额度；只有静默同步没能拿到可用会话时，才打开可见登录窗。
- [修复] 来源感知的受保护自愈：手动登录产生的会话在撞上登录横幅后先等 60 秒复核，确认过期才允许会话重复制覆盖。修复了一次横幅观测就摧毁刚登录 9 分钟的新会话的事故。
- [新功能] 会话确认失效且自动恢复手段用尽时，AACC 会自动弹出居中的 Qwen 登录窗，频率限制为每 30 分钟最多一次；用户主动退出登录后绝不自动弹窗。
- [修复] Qwen 可见登录窗现在居中打开，并在登录页加载前注入反检测（stealth）脚本。
- [修复] 更稳健的登录取消判定：短信验证、风控验证等跨域跳转不再触发 14 秒误自动取消；只有当托管浏览器的所有页面目标全部消失时才判定登录被取消。
- [修复] 零残留保证：每次操作结束后校验 AACC 专属 profile 没有任何残留 Chrome 进程，发现幸存者立即强制终止。
- [新功能] 额度栏新增双语状态文案：会话同步中、会话已过期自动恢复中。
